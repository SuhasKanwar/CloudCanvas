...
When a target config needs a value from a directly connected source node, use exactly ${node-id.outputName} and add a source-to-target edge. Supported output names are: EC2_INSTANCE instanceId; KEY_PAIR keyName, keyPairId; SECURITY_GROUP securityGroupId, groupName; ECR_REPOSITORY repositoryName, repositoryArn, repositoryUri; S3_BUCKET bucketName; IAM_ROLE roleName, roleArn, roleId; LAMBDA_FUNCTION functionName, functionArn; DYNAMODB_TABLE tableName, tableArn; SQS_QUEUE queueName, queueUrl; SNS_TOPIC topicName, topicArn. Do not use references without a direct edge.
