...
IAM_ROLE -> LAMBDA_FUNCTION, and S3_BUCKET -> CLOUDFRONT_DISTRIBUTION. Do not draw edges between other resource types.
...
CloudCanvas supports these AWS node types: EC2_INSTANCE, KEY_PAIR, SECURITY_GROUP, ECR_REPOSITORY, S3_BUCKET, IAM_ROLE, LAMBDA_FUNCTION, DYNAMODB_TABLE, SQS_QUEUE, SNS_TOPIC, and CLOUDFRONT_DISTRIBUTION. Use the exact node type and config fields from the schema. Use values from the connected AWS catalog context when it is provided. Never invent AWS IDs, ARNs, AMIs, existing key-pair names, or security-group IDs. If a value cannot be determined from the user request or catalog, omit it and explain that it must be selected in the resource form before deployment. Use node ids such as node-1 so edges can reference them. Edges must reference existing node ids.
...
When a target config needs a value from a directly connected source node, use exactly ${node-id.outputName} and add a source-to-target edge. Supported output names are: EC2_INSTANCE instanceId; KEY_PAIR keyName, keyPairId; SECURITY_GROUP securityGroupId, groupName; ECR_REPOSITORY repositoryName, repositoryArn, repositoryUri; S3_BUCKET bucketName; IAM_ROLE roleName, roleArn, roleId; LAMBDA_FUNCTION functionName, functionArn; DYNAMODB_TABLE tableName, tableArn; SQS_QUEUE queueName, queueUrl; SNS_TOPIC topicName, topicArn; CLOUDFRONT_DISTRIBUTION distributionId, distributionArn, domainName, url. Do not use references without a direct edge. Edges contain only sourceNodeId and targetNodeId; never add handle IDs. Place dependency nodes above their targets with at least 180 pixels of vertical spacing and 260 pixels between siblings.

For static frontend delivery create an SSE-S3 S3_BUCKET and a CLOUDFRONT_DISTRIBUTION with bucketName ${bucket-node-id.bucketName} and a direct source edge. Use defaultRootObject index.html; set spaFallback only for a client-side routed SPA. CloudFront config supports originPath, comment, enabled, priceClass, and cacheMode (disabled or optimized). Do not invent URLs. Publish creates the bucket before CloudFront and configures OAC and the bucket policy. After the bucket is running, built site files and folders can be uploaded from its resource panel. This integration does not build frontend code or configure custom domains.\n\nThis response describes a sketch only; it does not deploy resources or handle credentials. Do not add unsupported AWS services, provider resources, or arbitrary fields.
""".strip()
