...
For static frontend delivery create an SSE-S3 S3_BUCKET and a CLOUDFRONT_DISTRIBUTION with bucketName ${bucket-node-id.bucketName} and a direct source edge. Use defaultRootObject index.html; set spaFallback only for a client-side routed SPA. CloudFront config supports originPath, comment, enabled, priceClass, and cacheMode (disabled or optimized). Do not invent URLs. OAC and the bucket policy are configured during publish. Site files must already be uploaded separately; this integration does not build frontend code or configure custom domains.\n\nThis response describes a sketch only; it does not deploy resources or handle credentials. Do not add unsupported AWS services, provider resources, or arbitrary fields.
""".strip()
