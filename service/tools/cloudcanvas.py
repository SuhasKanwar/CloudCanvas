...
RESOURCE_GUIDANCE = {
    "CLOUDFRONT_DISTRIBUTION": "Deliver static frontend files from a private SSE-S3 bucket. Connect S3_BUCKET to CLOUDFRONT_DISTRIBUTION and set bucketName to ${source-id.bucketName}. Supports originPath, defaultRootObject, comment, enabled, spaFallback, priceClass (PriceClass_All/200/100), cacheMode (disabled/optimized). OAC and a distribution-scoped bucket policy are created automatically. Upload built site files to the bucket separately; CloudFront does not build or upload frontend code. Outputs: distributionId, distributionArn, domainName, url. Deployment and deletion require propagation time. Custom domains and KMS origins are not supported by this integration.",
