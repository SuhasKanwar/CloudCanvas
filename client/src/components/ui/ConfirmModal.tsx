...
export default function ConfirmModal({ cancelLabel = "Cancel", confirmLabel = "Confirm", confirming = false, description, onClose, onConfirm, open, title, variant = "neutral" }: { cancelLabel?: string; confirmLabel?: string; confirming?: boolean; description: string; onClose: () => void; onConfirm: () => void; open: boolean; title: string; variant?: ConfirmVariant }) {
    ...
    return <Modal description={description} dismissible={!confirming} onClose={onClose} open={open} size="compact" title={title}><div className="p-5 sm:p-6"><span className={`grid h-10 w-10 place-items-center rounded-md border ${style.tone}`}><Icon className="h-5 w-5" /></span><div className="mt-6 flex justify-end gap-3"><button className="border border-white/10 px-4 py-2 text-sm text-(--secondary-text-color) hover:bg-white/6 hover:text-(--primary-text-color) disabled:opacity-50" disabled={confirming} onClick={onClose} type="button">{cancelLabel}</button><button className={`px-4 py-2 text-sm font-medium disabled:opacity-50 ${style.button}`} disabled={confirming} onClick={onConfirm} type="button">{confirming ? "Working..." : confirmLabel}</button></div></div></Modal>;
}
