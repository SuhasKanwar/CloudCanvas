...
import { Bot, CheckCircle2, Loader2, Mic, PanelRightClose, Send, Sparkles, Square, Wand2 } from "lucide-react";
...
function ChatMessage({ message }: { message: SketchConversationMessage }) {
    const isUser = message.role === "USER";
    const nodeCount = message.type === "BUILD" && Array.isArray(message.build?.nodes) ? message.build.nodes.length : 0;
    return <motion.article animate={{ opacity: 1, y: 0 }} className={`max-w-[92%] border px-3 py-2.5 text-sm leading-6 ${isUser ? "ml-auto border-(--secondary-color)/45 bg-(--secondary-color) text-[#201508]" : "border-white/10 bg-white/4 text-(--primary-text-color)"}`} initial={{ opacity: 0, y: 5 }} transition={{ duration: 0.16 }}><div className="mb-1 flex items-center justify-between gap-4 text-[10px] font-medium uppercase tracking-[0.12em] opacity-65"><span>{isUser ? "You" : "CloudCanvas"}</span>{message.type === "BUILD" ? <span className="inline-flex items-center gap-1"><Wand2 className="h-3 w-3" />{nodeCount} node{nodeCount === 1 ? "" : "s"}</span> : null}</div>{isUser ? <p className="whitespace-pre-wrap">{message.content}</p> : <div className="[&_a]:text-(--secondary-color) [&_a]:underline [&_blockquote]:border-l-2 [&_blockquote]:border-white/20 [&_blockquote]:pl-3 [&_code]:bg-black/25 [&_code]:px-1 [&_pre]:my-2 [&_pre]:overflow-x-auto [&_pre]:bg-black/25 [&_pre]:p-3 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_p]:mb-2 [&_p:last-child]:mb-0" dangerouslySetInnerHTML={{ __html: md.render(message.content) }} />}{message.type === "BUILD" ? <div className="mt-3 flex items-center gap-2 border-t border-current/15 pt-2 text-xs"><CheckCircle2 className="h-3.5 w-3.5" />Blueprint saved to this conversation</div> : null}</motion.article>;
}
