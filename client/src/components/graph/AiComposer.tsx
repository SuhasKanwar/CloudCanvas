...
import { Bot, Check, CheckCircle2, GitBranch, Loader2, Mic, PanelRightClose, Send, Sparkles, Square, Trash2, Wand2 } from "lucide-react";
...
export default function AiComposer({ onApplyBlueprint, sketchId }: { onApplyBlueprint: (build: Omit<GraphDefinition, "schemaVersion">) => Promise<void>; sketchId: string }) {
    ...
    const [appliedBuildId, setAppliedBuildId] = useState<string | null>(null);
...
function ChatMessage({ applied, applying, message, onApply }: { applied: boolean; applying: boolean; message: SketchConversationMessage; onApply: () => void }) {
    const isUser = message.role === "USER";
    const nodeCount = message.type === "BUILD" && Array.isArray(message.build?.nodes) ? message.build.nodes.length : 0;
    const edgeCount = message.type === "BUILD" && Array.isArray(message.build?.edges) ? message.build.edges.length : 0;
    const services = message.type === "BUILD" ? [...new Set(message.build?.nodes.map((node) => awsServiceOptions.find((option) => option.service === node.type)?.title ?? node.type) ?? [])] : [];
    return <motion.article animate={{ opacity: 1, y: 0 }} className={`max-w-[92%] border px-3 py-2.5 text-sm leading-6 ${isUser ? "ml-auto border-(--secondary-color)/45 bg-(--secondary-color) text-[#201508]" : "border-white/10 bg-white/4 text-(--primary-text-color)"}`} initial={{ opacity: 0, y: 5 }} transition={{ duration: 0.16 }}><div className="mb-1 flex items-center justify-between gap-4 text-[10px] font-medium uppercase tracking-[0.12em] opacity-65"><span>{isUser ? "You" : "CloudCanvas"}</span>{message.type === "BUILD" ? <span className="inline-flex items-center gap-1"><Wand2 className="h-3 w-3" />Infrastructure blueprint</span> : null}</div>{isUser ? <p className="whitespace-pre-wrap">{message.content}</p> : <div className="[&_a]:text-(--secondary-color) [&_a]:underline [&_blockquote]:border-l-2 [&_blockquote]:border-white/20 [&_blockquote]:pl-3 [&_code]:bg-black/25 [&_code]:px-1 [&_pre]:my-2 [&_pre]:overflow-x-auto [&_pre]:bg-black/25 [&_pre]:p-3 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_p]:mb-2 [&_p:last-child]:mb-0" dangerouslySetInnerHTML={{ __html: md.render(message.content) }} />}{message.type === "BUILD" ? <div className="mt-3 border-t border-current/15 pt-2"><div className="flex items-center gap-4 text-[11px] text-(--secondary-text-color)"><span className="inline-flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-(--accent-color)" />{nodeCount} resource{nodeCount === 1 ? "" : "s"}</span><span className="inline-flex items-center gap-1.5"><GitBranch className="h-3.5 w-3.5 text-(--secondary-color)" />{edgeCount} dependenc{edgeCount === 1 ? "y" : "ies"}</span></div><p className="mt-1.5 truncate text-[11px] text-(--secondary-text-color)" title={services.join(", ")}>{services.join(" · ")}</p><div className="mt-2 flex items-center justify-between gap-3"><span className="text-[10px] text-(--muted-text-color)">Review configuration before publishing</span><button className="inline-flex h-8 shrink-0 items-center gap-1.5 border border-(--accent-color)/35 bg-(--accent-color)/10 px-2.5 text-[11px] font-medium text-(--accent-color) transition hover:border-(--accent-color) hover:bg-(--accent-color)/15 disabled:opacity-50" disabled={applying || applied} onClick={onApply} type="button">{applying ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : applied ? <Check className="h-3.5 w-3.5" /> : <Wand2 className="h-3.5 w-3.5" />}{applying ? "Applying" : applied ? "Applied" : "Apply blueprint"}</button></div></div> : null}</motion.article>;
}
