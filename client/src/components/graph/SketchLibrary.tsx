...
import { deleteSketch, getSketch, listSketches, type Sketch } from "@/lib/sketches";
import { useOutsideDismiss } from "@/hooks/useOutsideDismiss";

export default function SketchLibrary({ onDelete, onLoad }: { onDelete: (sketchId: string) => void; onLoad: (sketch: Sketch) => void }) {
    ...
    return <div className="relative" ref={libraryRef}>
        ...
        {open ? <div className="absolute right-0 top-11 z-30 w-72 border border-white/12 bg-[#151821] shadow-2xl">
            ...
            <div className="max-h-72 overflow-auto">
                ...
                {sketches.map((sketch) => <div className="flex border-b border-white/8" key={sketch.id}><button className="min-w-0 flex-1 px-4 py-3 text-left hover:bg-white/6" onClick={() => void load(sketch.id)} type="button"><span className="block truncate text-sm text-(--primary-text-color)">{sketch.name}</span><span className="mt-1 block font-mono text-[10px] uppercase text-(--secondary-text-color)">{sketch.status}</span></button><button aria-label={`Delete ${sketch.name}`} className="px-3 text-(--secondary-text-color) hover:bg-white/6 hover:text-(--danger-color)" onClick={() => void remove(sketch)} title="Delete sketch" type="button"><Trash2 className="h-4 w-4" /></button></div>)}
            </div>
        </div> : null}
    </div>;
}
