...
type FieldProps = { label: string; value: string | number; onChange: (value: string) => void; disabled?: boolean; type?: "number" | "text" };
...
function DynamoDbForm({ config, update, deployed }: { deployed: boolean; config: Record<string, unknown>; update: (key: string, value: unknown) => void }) {
    ...
    return <>
        <fieldset disabled={deployed} className="space-y-5 disabled:opacity-55"><Field label="Table name" onChange={(value) => update("tableName", value)} value={String(config.tableName ?? "")} />
        ...
        </fieldset><Select label="Billing mode" onChange={(value) => update("billingMode", value)} options={[["PAY_PER_REQUEST", "On-demand"], ["PROVISIONED", "Provisioned"]]} value={String(config.billingMode ?? "PAY_PER_REQUEST")} />
...
export default function ResourceInspector({ bindings, connectionId, node, resource, onChange, onDelete, onOpenAwsSettings }: Props) {
    ...
    const formLocked = deployed && !["EC2_INSTANCE", "S3_BUCKET", "LAMBDA_FUNCTION", "ECR_REPOSITORY", "DYNAMODB_TABLE", "SQS_QUEUE"].includes(service);
