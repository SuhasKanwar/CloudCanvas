...
import { Braces, Code2, Tag } from "lucide-react";
...
export default function ResourceInspector({ node, onChange }: { node: Node<ResourceNodeData>; onChange: (label: string, config: Record<string, unknown>) => void }) {
    ...
    return <div className="h-full overflow-auto p-5">
        <div className="flex items-center gap-2 text-sm font-medium text-(--primary-text-color)"><Code2 className="h-4 w-4 text-(--secondary-color)" />Resource settings</div>
